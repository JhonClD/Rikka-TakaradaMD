...
const handler = async (m, { conn, command, usedPrefix }) => {
  try {
    ...
    if (me.lastClaim && now < me.lastClaim) {
      ...
      let t = minutes > 0 ? `${minutes} minuto${minutes !== 1 ? 's' : ''} ` : '';
      t += `${seconds} segundo${seconds !== 1 ? 's' : ''}`;
      return m.reply(`ꕥ Debes esperar *${t.trim()}* para usar *${command}* de nuevo.`);
    }
    ...
    await conn.sendMessage(m.chat, { text: `❀ ${finalMessage} (${duration}s)` }, { quoted: m });
