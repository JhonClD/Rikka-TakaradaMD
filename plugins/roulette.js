// roulette.js — Portado de YukiBot-MD → Rikka-TakaradaMD
...
const handler = async (m, { conn, command, usedPrefix, args }) => {
    ...
    const currency = botSettings?.currency || 'Yenes' || 'Monedas'
    ...
    if (resultColor === color) {
      ...
      user.coins += reward
      await conn.sendMessage(chatId, { text: `「✿」 La ruleta salió en *${resultColor}* y has ganado *¥${reward.toLocaleString()} ${currency}*.`, mentions: [senderId] }, { quoted: m })
    } else {
      user.coins -= amount
      await conn.sendMessage(chatId, { text: `「✿」 La ruleta salió en *${resultColor}* y has perdido *¥${amount.toLocaleString()} ${currency}*.`, mentions: [senderId] }, { quoted: m })
    }
};

handler.command = ['rt', 'roulette', 'ruleta'];
