// aceptar.js — Portado de YukiBot-MD → Rikka-TakaradaMD
...
const handler = async (m, { conn, command, usedPrefix, args }) => {
    try {
      ...
      if (intercambio) {
        ...
        return await conn.sendMessage(chatId, { text: mensajeConfirmacion }, { quoted: m })
      }
      ...
      if (regalo) {
        ...
        await conn.sendMessage(chatId, { text: `「✿」 Has regalado con éxito todos tus personajes a ${name}!\n\n> ❏ Personajes regalados: ${regalo.count}\n> ⴵ Valor total: ${regalo.value.toLocaleString()}` }, { quoted: m })
