...
const handler = async (m, { conn, text, args, usedPrefix, command }) => {
  ...
  // Resolver URL corta (vt.tiktok.com, vm.tiktok.com) → URL larga
  let url = args[0];
  try {
    ...
    if (res.request?.res?.responseUrl) url = res.request.res.responseUrl;
    else if (res.headers?.location)    url = res.headers.location;
