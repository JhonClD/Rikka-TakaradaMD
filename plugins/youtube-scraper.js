...
// Si no hay ffmpeg instalado en el sistema, usa ffmpeg-static (incluido en node_modules)
// Esto permite que el bot funcione sin instalar ffmpeg manualmente.
...
try {
    ...
    try {
        // Intentar usar ffmpeg-static como fallback
        const { default: ffmpegStatic } = await import('ffmpeg-static');
        if (ffmpegStatic && fs.existsSync(ffmpegStatic)) {
            FFMPEG_BIN = ffmpegStatic;
            // ffprobe-static si está disponible
            try {
                const { default: ffprobeStatic } = await import('@ffprobe-installer/ffprobe');
                if (ffprobeStatic?.path && fs.existsSync(ffprobeStatic.path)) {
                    FFPROBE_BIN = ffprobeStatic.path;
                }
            } catch { /* ffprobe-static no instalado, seguir con el del sistema */ }
            console.log(`[youtube-scraper] ✅ Usando ffmpeg-static: ${FFMPEG_BIN}`);
        }
    } catch {
        console.warn('[youtube-scraper] ⚠️  ffmpeg no encontrado. Instala ffmpeg o agrega ffmpeg-static al package.json');
    }
}
