...
global.reloadHandler = async function (restatConn) {
  ...
  // ── FIX LID: poblar caché de LIDs cuando se unen/actualizan grupos ──
  conn.ev.on('groups.upsert', async (groups) => {
    for (const group of groups) {
      if (group?.participants?.length) {
        const cached = lidResolver.bulkCacheFromParticipants(group.participants);
        if (cached > 0) console.log(`[ 🆔 ] Cacheados ${cached} LIDs del grupo ${group.id}`);
      }
    }
  });

  // ── FIX LID: poblar al agregar participantes ──
  conn.ev.on('group-participants.update', async ({ id, participants, action }) => {
    if ((action === 'add' || action === 'promote') && participants?.length) {
      try {
        const meta = await conn.groupMetadata(id).catch(() => null);
        if (meta?.participants) lidResolver.bulkCacheFromParticipants(meta.participants);
      } catch (_) {}
    }
  });
