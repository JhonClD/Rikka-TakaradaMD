...
export function registerLidPhone(lid, phoneJid) {
  if (lid && phoneJid) _cache.set(lid, phoneJid.split('@')[0]);
}
...
function resolveFromContacts(lid, conn) {
  const sources = [conn?.contacts, global?.conn?.contacts];
  for (const source of sources) {
    if (!source || typeof source !== 'object') continue;
    for (const [jid, contact] of Object.entries(source)) {
...
export function resolveJidToPhone(jid, conn) {
  ...
  if (isLidJid(jid)) {
    ...
    return resolveFromGroupCache(jid) ?? resolveFromContacts(jid, conn) ?? null;
  }
  return jid.split('@')[0];
}
...
export function normalizeSender(jid, conn) {
  if (!jid) return '';
  if (isLidJid(jid)) {
