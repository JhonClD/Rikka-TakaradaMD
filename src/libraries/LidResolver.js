...
class LidResolver {
      ...
      while (attempts < maxRetries) {
        try {
          ...
          for (const participant of metadata.participants) {
            try {
              // Fix: este fork de Baileys usa participant.id, no participant.jid
              const pJid = participant?.id || participant?.jid;
              if (!pJid) continue;

              // Método 1 (directo): participant.lid está disponible en este fork de Baileys
              ...
              // Método 2 (fallback): intentar via onWhatsApp
  ...
  async processMessage(message) {
    try {
      ...
      // ── NUEVO: resolver LID en DM usando caché existente ─────────────────
      // En chats privados no hay groupChatId, pero el remoteJid o participant
      // puede ser un LID. Intentamos resolverlo desde el caché local.
      if (!groupChatId) {
        const processedMessage = JSON.parse(JSON.stringify(message));
        let changed = false;

        // remoteJid como LID (DM con usuario LID)
        if (processedMessage.key?.remoteJid?.endsWith('@lid')) {
          ...
          if (cached?.jid && !cached.jid.endsWith('@lid')) {
            ...
            // Intentar resolver directamente sin grupo (onWhatsApp)
            try {
  ...
  /**
   * Poblar caché en masa desde array de participantes de grupo
   * Usa participant.lid directamente (disponible en este fork de Baileys)
   */
