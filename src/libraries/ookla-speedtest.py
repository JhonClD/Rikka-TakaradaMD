...
class Speedtest(object):
    ...
    def get_closest_servers(self, limit=3):
...
def parse_args():
    ...
    parser.add_argument('--fast', action='store_true', default=False,
                        help='Reduce test duration to ~5s each for faster results')
...
def shell():
    ...
    # --fast: reduce test length to 5s each (default is ~10s from server config)
    if getattr(args, 'fast', False):
        speedtest.config['length']['download'] = 5
        speedtest.config['length']['upload']   = 5
